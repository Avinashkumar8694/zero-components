...
export class ZeroFlowNodeStart extends LitElement {
  ...
  @RendererAttribute({ attributeType: AttributeType.PROPERTY, uiComponentType: UserInterfaceType.TEXTAREA, displayLabel: "Discovery Hint", fieldMappings: "entryUsage" }) get entryUsageConfig() { return this.entryUsage; } set entryUsageConfig(value: string) { this.entryUsage = value || ""; }
  render() { return html`<div class="node"><div class="eyebrow">Entry Point</div><strong>${this.entryRole}</strong><div style="margin-top:4px;font-size:0.8rem;color:#64748b">${this.entryUsage || "This node is an entry point discoverable by the Studio."}</div></div>`; }
}
