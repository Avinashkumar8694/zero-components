...
export class SchemaRenderer {
  ...
  private renderResolvedNode(node: UIComponentNode, scope: Record<string, unknown>, renderId: string): TemplateResult {
    ...
    if (resolvedNode.componentName === "page-root") {
      ...
      const forcedInlineStyle = serializeStyles(pageStyles);
      return html`${responsiveCss ? html`<style>${responsiveCss}</style>` : null}<section data-node-id=${renderId} style=${forcedInlineStyle}>${children}</section>`;
    }
    ...
    if (resolvedNode.componentName === "zero-page-ref") {
      ...
      if (typeof pageId === "string" && pageId && this.activeSchema) {
        ...
        if (matchedPage) {
          const pageRefProps = resolvedNode.props ?? {};
          const extendedScope = {
            ...scope,
            page: {
              ...(scope.page as any),
    ...
    let versionedTag = `${baseName}-${version}`;
    let actualTag = baseName;

    // Use the versioned tag if it's registered, otherwise fall back to base name
    let isVersionedRegistered = !!customElements.get(versionedTag);
    let isBaseRegistered = !!customElements.get(baseName);

    if (!isVersionedRegistered && !isBaseRegistered) {
      // Look up in zero registry to see if there's a custom element selector mapping
      const zeroRegistry = (window as any).zero || (window as any).ZeroRegistry;
      if (zeroRegistry?.components) {
        const allComps = Object.values(zeroRegistry.components) as any[];
        const found = allComps.find((comp) => 
          comp.metadata?.name === baseName || 
          comp.metadata?.selector === baseName || 
          comp.metadata?.elementSelector === baseName ||
          comp.metadata?.name === baseName.replace("zero-", "")
        );
        if (found && found.metadata) {
          const selector = found.metadata.selector || found.metadata.elementSelector || baseName;
          versionedTag = `${selector}-${version}`;
          isVersionedRegistered = !!customElements.get(versionedTag);
          isBaseRegistered = !!customElements.get(selector);
          actualTag = isVersionedRegistered ? versionedTag : selector;
        }
      }
    } else {
      actualTag = isVersionedRegistered ? versionedTag : baseName;
    }
...
function applyProps(
  ...
  // For plain HTML elements, apply all props directly as JS properties
  ...
  // For custom elements (Lit / Web Components) — apply all props directly.
  // We set both JS properties (for upgraded Lit elements) and HTML attributes
  // (for boolean reflect props like `expanded`, `expandable`, `visible`) so
  // that CSS `:host([attr])` selectors work as expected.
  for (const [key, value] of Object.entries(props)) {
    try {
      // Always set JS property — Lit @property() will react to this
      element[key] = value;

      // Also sync booleans and primitives as attributes so CSS selectors work
      if (value === null || value === undefined) {
      ...
      // Ignore read-only properties or setters that throw
    }
  }
}

export function findNode(root: UIComponentNode, nodeId: string): UIComponentNode | null {
  if (root.id === nodeId) {
    return root;
  }
