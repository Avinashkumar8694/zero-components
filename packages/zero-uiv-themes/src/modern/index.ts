...
export class ModernTheme extends BaseTheme {
    id = 'modern';
    name = 'Modern Minimal';
    globalTokens = modernGlobalTokens;
    componentTokens = modernComponentTokens;
    baseStyles = css`
        :host {
        ...
        /* Standard aliases for component consumption */
        .uiv-modern-card {
            background: var(--uiv-bg-surface);
            border: 1px solid var(--uiv-border-color);
            border-radius: 12px;
    ...
    getComponentStyles(componentName: string) {
        return css`:host { ${createThemeStyles(this.getComponentTokens(componentName))} }`;
    }
}
